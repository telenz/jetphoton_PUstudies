...
int plotHistograms(){
  ...
  //fileName = "root_files/response_intrinsic_jet_in_12_Pt_bin_1_eta_bin_1_alpha_bin__PFCHS_mc.root";
  //fileName = "hAlpha_PFCHS_mc.root";
  //fileName = "root_files/hDeltaPhi_PFCHS_mc.root";
  fileName = "root_files/response_jet_in_12_Pt_bin_1_eta_bin_6_alpha_bin__PF_mc.root";
  file = TFile::Open(fileName);
  file -> GetObject("histo1506",histo);

  //file -> GetObject("histo1422",histo);
  //file -> GetObject("hAlpha",histo);
  //file -> GetObject("hDeltaPhi",histo);
  //file -> GetObject("histo1506",histoAdd);
  //delete file; 

  cout<<"color = "<<histo->GetLineColor()<<endl;
  
  fileName = "root_files/response_photon_in_12_Pt_bin_1_eta_bin_6_alpha_bin__PF_mc.root";
  file = TFile::Open(fileName);
  file -> GetObject("histo1514",histo1);

  float integralSum = histo->Integral() + histo1->Integral();
  
  histo->Scale((histo->GetEntries()+histo1->GetEntries())/integralSum);
  histo1->Scale((histo->GetEntries()+histo1->GetEntries())/integralSum);
  ...
  histoAdd = (TH1D*)histo->Clone();
  histoAdd -> Add(histo1);
  
  

  //histo -> GetXaxis()->SetTitle("#p_{T}^{rekon. Jet}/p_{T}^{gen. Jet}");
  //histo -> GetXaxis()->SetTitle("#Delta #Phi(Jet_{1},#gamma)");
  histo -> GetXaxis()->SetTitle("p_{T}^{rekon. Jet}/p_{T}^{#gamma}");
  histo -> SetTitleSize(0.05,"X");
  histo -> GetYaxis()->SetTitle("# Ereignisse");
  histo -> SetTitleSize(0.05,"Y");
  
  canvas -> SetBottomMargin(0.15);
  canvas -> SetLeftMargin(0.2);
  // histo -> GetXaxis()->SetLimits(0.6,1.4);
  //histo -> GetXaxis()->SetLimits(0.0,1.0);
  histo -> GetXaxis()->SetLimits(0,2);
  histo->SetMinimum(1);
  histo->SetMaximum(5*1000);
  histo -> SetTitle("");
  ...
  //info->DrawLatex(0.62,0.65,  "p_{T}^{gen Jet} > 400 GeV");
  info->DrawLatex(0.64,0.72,  "#eta^{Jet_{1}} < 0.5 ");
  info->DrawLatex(0.64,0.66,  "p_{T}^{gen Jet} > 400 GeV");
  info->DrawLatex(0.24,0.72,  "0.175 < #alpha < 0.20");
