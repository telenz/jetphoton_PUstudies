...
int plotHistograms(){
  ...
  //fileName = "root_files/response_jet_in_12_Pt_bin_1_eta_bin_6_alpha_bin__PFCHS_mc.root";
  ...
  cout<<"1"<<endl;
  fileName = "root_files/response_photon_in_12_Pt_bin_1_eta_bin_6_alpha_bin__PFCHS_mc.root";
  fileName = "root_files/hPhoton1Pt.root";
  fileName = "root_files/hJet2Pt.root";
  fileName = "root_files/hDeltaPhi.root";
  file = TFile::Open(fileName);
  cout<<"2"<<endl;
  ...
  histo -> GetXaxis()->SetLimits(2,3.2);
  //histo -> GetXaxis()->SetLimits(0,100);
  //histo->SetMinimum(1);
  //histo -> SetTitle("2nd Jet p_{T} (after all cuts)");
  histo -> SetTitle("#Delta#Phi (before #Delta#Phi - cut)");
  ...
  //info->DrawLatex(0.64,0.65,  "#splitline{#eta^{Jet_{1}} < 0.5}{p_{T}^{gen Jet} > 400 GeV}");
  //info->DrawLatex(0.24,0.75,  "0.0 < #alpha < 0.075");
