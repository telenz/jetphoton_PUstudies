...
int makeGraphData(){
  ...
  for(int j=0; j<1; j++){
    ...
    canvas[i] ->cd();
    fileName.Form("root_files_JetHemisphere/jet_energy_resolution_for_1_eta_bin_%i_pTGamma_bin_imbalance_PFCHS_mc.root",i+1);
    //fileName.Form("root_files_JetHemisphere/jet_energy_resolution_for_1_eta_bin_%i_pTGamma_bin_PFCHS_mc.root",i+1);
    ...
    histoJet -> SetMarkerStyle(20);
    histoJet -> SetLineColor(2);
    histoJet -> SetLineColor(2);
    histoJet -> GetXaxis()->SetTitleSize(0.06);
    histoJet -> GetYaxis()->SetTitleSize(0.06);
    histoJet -> GetXaxis()->SetTitleOffset(0.9);
    histoJet -> GetYaxis()->SetTitleOffset(1.);
    histoJet->SetTitle("Imbalance (:= p_{T}^{gen. Jet}/ p_{T}^{#gamma})");
    //histoJet->SetTitle("Full Resolution (:= p_{T}^{reco. Jet}/ p_{T}^{#gamma})");
    ...
    fileName.Form("root_files_PhotonHemisphere/jet_energy_resolution_for_1_eta_bin_%i_pTGamma_bin_imbalance_PFCHS_mc.root",i+1);
    //fileName.Form("root_files_PhotonHemisphere/jet_energy_resolution_for_1_eta_bin_%i_pTGamma_bin_PFCHS_mc.root",i+1);
    file =  TFile::Open(fileName);
    file -> GetObject("Graph",histoPhoton);
    histoPhoton -> SetMarkerColor(3);
    histoPhoton -> SetLineColor(3);
    histoPhoton -> SetMarkerStyle(20);
    ...
    TLegend *legend  = new TLegend(0.2,0.8,0.6,0.9);
    ...
    info->DrawLatex(0.44,0.2,  EtaPtRegion);
    

    fileName.Form("jet_energy_resolution_for_1_eta_bin_%i_ptGamma_bin_imbalance_comparison.pdf",i+1);
    //fileName.Form("jet_energy_resolution_for_1_eta_bin_%i_ptGamma_bin_comparison.pdf",i+1);
    canvas[i] -> SetBottomMargin(0.14);
    canvas[i] -> SetLeftMargin(0.14);
