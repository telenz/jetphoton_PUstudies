...
int makeGraphData(){
  ...
  TF1 *fResolutionOld[4];
  TF1 *fResolutionNew[4];
  TGraph *diff[4];
  TCanvas* canvas[4];
  TCanvas* canvas1[4];
  ...
  for(int i =0; i<4; i++){
    ...
    fResolutionOld[i] = new TF1("fResolution","TMath::Sqrt(TMath::Sign(1,[0])*TMath::Power([0]/x,2)+TMath::Power([1],2)*TMath::Power(x,[3]-1)+TMath::Power([2],2))", 80, 500);
    if(i==0){
      fResolutionOld[i] -> SetParameter(0,-1.13);
      fResolutionOld[i] -> SetParameter(1,0.611);
      fResolutionOld[i] -> SetParameter(2,0.03121);
      fResolutionOld[i] -> SetParameter(3,0.143);
    }
    else if(i==1){
      fResolutionOld[i] -> SetParameter(0,-0.44);
      fResolutionOld[i] -> SetParameter(1,0.625);
      fResolutionOld[i] -> SetParameter(2,0.03401);
      fResolutionOld[i] -> SetParameter(3,0.136);
    }
    else if(i==2){
      fResolutionOld[i] -> SetParameter(0,-3.20);
      fResolutionOld[i] -> SetParameter(1,1.529);
      fResolutionOld[i] -> SetParameter(2,0.04100);
      fResolutionOld[i] -> SetParameter(3,-0.196);
    }
    else if(i==3){
      fResolutionOld[i] -> SetParameter(0,-3.37);
      fResolutionOld[i] -> SetParameter(1,2.08);
      fResolutionOld[i] -> SetParameter(2,0.0321);
      fResolutionOld[i] -> SetParameter(3,-0.416);
    }


    fileName.Form("root_files/Resolution_for_%i_eta_bin_intrinsic_PF_mc.root",i+1);

    file = TFile::Open(fileName);
    file->GetObject("Graph",Graph);

    fResolutionNew[i] = new TF1("fResolution","TMath::Sqrt(TMath::Sign(1,[0])*TMath::Power([0]/x,2)+TMath::Power([1],2)*TMath::Power(x,[3]-1)+TMath::Power([2],2))", 80, 500);

    fResolutionNew[i] -> SetParameter(0,Graph->GetFunction("fResolution")->GetParameter(0));
    fResolutionNew[i] -> SetParameter(1,Graph->GetFunction("fResolution")->GetParameter(1));
    fResolutionNew[i] -> SetParameter(2,Graph->GetFunction("fResolution")->GetParameter(2));
    fResolutionNew[i] -> SetParameter(3,Graph->GetFunction("fResolution")->GetParameter(3));

    cout<<Graph->GetFunction("fResolution")->GetParameter(0)<<endl;
    cout<<Graph->GetFunction("fResolution")->GetParameter(1)<<endl;
    cout<<Graph->GetFunction("fResolution")->GetParameter(2)<<endl;
    cout<<Graph->GetFunction("fResolution")->GetParameter(3)<<endl<<endl;

    fileName.Form("canvas_%i",i+1);  
    canvas[i] = new TCanvas(fileName,fileName,10,10,500,500);
    canvas[i] ->cd();
  
    fResolutionOld[i] -> Draw();
    fResolutionNew[i] -> SetLineColor(3);
    fResolutionNew[i] -> Draw("same");

    TLatex*  info   = new TLatex();
    info->SetTextFont(132);
    info-> SetNDC();
    info->SetTextSize(0.035);
    info->DrawLatex(0.22,0.84,"#splitline{1. #eta bin}{2 #sigma gaussian fit}");

    TLegend *legend  = new TLegend(0.5,0.8,0.9,0.9);
    legend -> SetFillColor(0);
    legend -> SetTextSize(0.033);
    legend -> AddEntry(fResolutionOld[i],"2011","l");
    legend -> AddEntry(fResolutionNew[i],"my Results 2011","l");
    legend -> Draw("same");
   
    fileName.Form("Comparison_of_2011_results_%i_etaBin.pdf",i+1);
    ...
    for(int j=0;j<100;j++){
      ...
      x[j] = 4.5*j+80;
      y[j] = fResolutionNew[i] -> Eval(x[j]);
      y[j] = y[j]/fResolutionOld[i] -> Eval(x[j]) - 1.;

    }

    diff[i] = new TGraph(100,x,y);

    fileName.Form("canvas1_%i",i+1);  
    canvas1[i] = new TCanvas(fileName,fileName,10,10,500,500);
    canvas1[i] ->cd();
    diff[i]->SetMarkerStyle(20);
    diff[i]->Draw("AP");
    info->DrawLatex(0.62,0.84,"#splitline{1. #eta bin}{2 #sigma gaussian fit}");

    fileName.Form("Ratio_of_2011_results_%i_etaBin.pdf",i+1);
    canvas1[i] ->SaveAs(fileName);
  }
